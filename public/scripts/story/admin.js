...
function setup() {
	...
	.then(data => {
		...
		let formBody = $create(`<form method = "POST" action = "/api/edit-story" enctype = "multipart/form-data" class = "form-body">
		...
		formBody.appendChild($create(`<button type = "submit" formaction = "/api/delete-story">Delete</button>`));
		...
		content.replaceWith($create(`<p><textarea name = "story" id = "content">${document.createTextNode(data.content).textContent}</textarea></p>`));
		...
		image.innerHTML = `
			<input id = "story-image" type = "file" name = "file">
			<label for = "story-image" id = "select-image" style = "background : transparent;">+</label>`;
		image.style.backgroundImage = `url("${data.image}")`;

		let input = document.getElementById("story-image");
		...
		function save() {
			if(!hasSaved) {
				let submit = $create(`<button type = "submit" formaction = "/api/edit-story">Save Edit</button>`);
		...
		//{ create reply form
			...
			let replyForm = $create(`<form class = "reply-form" method = "POST" action = "/story-post">
					<input type = "hidden" name = "target" value = "story-create">

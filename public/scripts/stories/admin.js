...
.then(data => {
	...
	let addStory = document.createElement("div");

	addStory.classList.add("book-card");
	addStory.classList.add("create-story");

	addStory.innerHTML = `<a href = "/create-story">+</a>`;

	bookCards.appendChild(addStory);
})
.catch(err => {
	console.log(err);
});
...
.then(data => {
	...
	let addWrite = document.createElement("div");

	addWrite.classList.add("writing-card");
	addWrite.classList.add("create-write");

	addWrite.innerHTML = `<a href = "/create-write">+</a>`;

	quickWriteCards.appendChild(addWrite);
})
.catch(err => {
	console.log(err);
});